...
interface IEthereumVaultConnector {
    ...
    error EVC_RevertedBatchResult(
        BatchItemResult[] batchItemsResult, StatusCheckResult[] accountsStatusResult, StatusCheckResult[] vaultsStatusResult
    );
    ...
    function controlCollateral(address targetCollateral, address onBehalfOfAccount, uint256 value, bytes memory data)
        ...
        payable
        returns (bytes memory result);
    function disableCollateral(address account, IEulerVault vault) external payable;
    ...
    function isCollateralEnabled(address account, IEulerVault vault) external view returns (bool);
    function isControlCollateralInProgress() external view returns (bool);
    function isControllerEnabled(address account, IEulerVault vault) external view returns (bool);
    ...
    function isVaultStatusCheckDeferred(IEulerVault vault) external view returns (bool);
